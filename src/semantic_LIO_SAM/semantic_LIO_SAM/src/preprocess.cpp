...
void Preprocess::velodyne_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    ...
    // KITTI等数据常见仅有 x/y/z/intensity。缺少ring/time时走兼容路径，避免整帧被丢弃。
    if (!has_ring || !has_time) {
      pcl::PointCloud<pcl::PointXYZI> pl_xyzi;
      pcl::fromROSMsg(*msg, pl_xyzi);
      const int plsize = static_cast<int>(pl_xyzi.points.size());
      if (plsize <= 0) return;
      pl_surf.reserve(plsize);
      const float scan_period_ms = 1000.0f / std::max(1, SCAN_RATE);
      for (int i = 0; i < plsize; ++i) {
        if (i % point_filter_num != 0) continue;
        const auto& p = pl_xyzi.points[i];
        ...
        added_pt.curvature = (plsize > 1)
                                 ? (static_cast<float>(i) / static_cast<float>(plsize - 1)) * scan_period_ms
                                 : 0.0f;
        pl_surf.points.push_back(added_pt);
      }
      return;
    ...
    else
    {
      for (int i = 0; i < plsize; i++)
      {
        ...
        int layer = pl_orig.points[i].ring;
        if (layer < 0 || layer >= N_SCANS) continue;
